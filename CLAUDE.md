...
2. Network (VNet, subnets, NSGs, private DNS zones) ← **Complete (2026-06-15)** — infra applied 2026-06-11; service doc closed 2026-06-15. `network.tf` written, reviewed, plan+apply clean (18 resources). Changelog done. **CIDR ground truth (verified against tfstate 2026-06-14):** VNet `10.0.0.0/16`, `snet-aca` `10.0.0.0/24`, `snet-pg` `10.0.1.0/28`, `snet-pe` `10.0.2.0/27`. The earlier `10.10.0.0/16` + `/23/24/27` scheme was the **abandoned first-pass design** — it is NOT deployed; ignore it wherever it lingers (e.g. `docs/changelog/phase-2-first-pass.md`, which is kept as historical record). `architecture.md` was reconciled to this ground truth on 2026-06-14 (current + target diagrams, topology diagram, and current/target table — also removed Postgres/ACR which the diagram wrongly showed as deployed). **Closed (2026-06-15):** `docs/services/azure-network.md` was written into a full service doc across all nine template sections (Brian drafted, Claude edited) using the verified CIDRs above — Module 2 complete. Next up: Module 3 (Key Vault) per `docs/mentor/m3-s1-key-vault.md`.
3. Key Vault ← **Infra applied 2026-06-28; docs pending.** `infra/environments/prod/keyvault.tf` written, reviewed, and applied clean (`Plan: 4 to add` → applied; re-apply shows `0 changed`). All 2026-06-21 review fixes landed (3 blockers + 3 should-fixes), plus two found at `validate`: tfvars key `home_ip`→`deployer_ip`, and `enable_rbac_authorization`→`rbac_authorization_enabled` (deprecated in azurerm, removed v5.0). **Live facts:** vault `kv-lifestack-prod-tx9d` in `rg-lifestack-app-prod`; PE `pe-kv-lifestack-prod` at private IP `10.0.2.4` (in `snet-pe`), A record auto-created in the `kv` private DNS zone — first PE in the project, verified via `terraform state show`. Decisions [D1]–[D5] locked: RG=`app`, standard SKU, RBAC + deployer `Key Vault Secrets Officer`, public-with-firewall+PE (allow deployer `/32`), purge protection OFF + 7-day retention. **Remaining to close Module 3 (Brian drafts, Claude edits):** (a) service doc `docs/services/azure-key-vault.md` across all 9 template sections; (b) `docs/changelog.md` entry; (c) optional ADR for the public-with-firewall+PE network model (documented deviation from the PE-only prod ideal); (d) flip this line + footer to **Complete**. **Cross-device note:** `deployer_ip` lives in `terraform.tfvars` (gitignored, does NOT sync). On the other device, copy `terraform.tfvars.example` → `terraform.tfvars`, set `subscription_id` + that device's own public IP (`curl -s ifconfig.me`); the vault's `ip_rules` currently allows only `99.233.21.112/32`, so to write secrets (Module 6+) from a different network you must update `deployer_ip` and re-apply the vault. State syncs automatically via the remote backend (`terraform init` connects to `stlifestack9k3l`).
