...
2. Network (VNet, subnets, NSGs, private DNS zones) ← **Infra applied (2026-06-11); docs nearly closed (2026-06-14)** — `network.tf` written, reviewed, plan+apply clean (18 resources). Changelog done. `architecture.md` network topology section **done** (current + target diagrams, topology section). **Still open:** `docs/services/azure-network.md` exists only as a rough outline and its CIDRs are stale (says 10.0.0.0/16, /24/28/27 — actual applied infra is 10.10.0.0/16, /23/24/27); Brian still needs to write it into a full service doc (Brian drafts, Claude edits) to close Module 2.
