...
3. Key Vault ← **In progress — draft reviewed 2026-06-21.** Mentor file `docs/mentor/m3-s1-key-vault.md` covers [D1]–[D5] + the 5 resources (random suffix, key_vault, deployer role assignment, **first private endpoint in the project**, PE wiring). Decisions made (in the draft): RG=`app`, SKU=standard, RBAC auth + deployer `Key Vault Secrets Officer` role, public-with-firewall+PE network model (allow deployer `/32`), purge protection OFF + 7-day retention. `infra/environments/prod/keyvault.tf` is drafted and reviewed but **not yet planned** — see the "Review status — 2026-06-21" section at the bottom of the mentor file for the open fixes: 3 blockers (PE missing both nested blocks; `ip_rules` references nonexistent `home_ip` w/ bad interpolation, should be `deployer_ip` + `/32`; `sku_name` should be lowercase `standard`) and 3 should-fixes (set `public_network_access_enabled = true` explicitly; add `tags` to the PE; run `terraform fmt`). **Next action:** apply those fixes, `terraform fmt`, then `plan` (expect "4 to add"). `deployer_ip` lives in `terraform.tfvars` (gitignored) — get it with `curl -s ifconfig.me`.
