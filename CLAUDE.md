...
2. Network (VNet, subnets, NSGs, private DNS zones) ← **Infra applied (2026-06-11); docs nearly closed (2026-06-14)** — `network.tf` written, reviewed, plan+apply clean (18 resources). Changelog done. **CIDR ground truth (verified against tfstate 2026-06-14):** VNet `10.0.0.0/16`, `snet-aca` `10.0.0.0/24`, `snet-pg` `10.0.1.0/28`, `snet-pe` `10.0.2.0/27`. The earlier `10.10.0.0/16` + `/23/24/27` scheme was the **abandoned first-pass design** — it is NOT deployed; ignore it wherever it lingers (e.g. `docs/changelog/phase-2-first-pass.md`, which is kept as historical record). `architecture.md` was reconciled to this ground truth on 2026-06-14 (current + target diagrams, topology diagram, and current/target table — also removed Postgres/ACR which the diagram wrongly showed as deployed). **Still open:** `docs/services/azure-network.md` exists only as a rough outline; Brian still needs to write it into a full service doc (Brian drafts, Claude edits) using the verified CIDRs above to close Module 2.
3. Key Vault ← **Mentor file written (2026-06-14)** — `docs/mentor/m3-s1-key-vault.md` covers [D1]–[D5] + the 5 resources (random suffix, key_vault, deployer role assignment, **first private endpoint in the project**, PE wiring). Recommendations baked in: RG=`app`, SKU=standard, RBAC auth, public-with-firewall+PE network model (allow your `/32`), purge protection OFF + 7-day retention (learning-env choice). **Next action:** Brian writes `infra/environments/prod/keyvault.tf` from scratch, fills [D1]–[D5], runs `terraform fmt`, sends for review **before** `plan`. Get public IP first (`curl -s ifconfig.me`). Plan should be "4 to add."
...
*Last updated: 2026-06-14 — Cross-device handoff + CIDR reconciliation. **CIDR correction:** an earlier handoff had the discrepancy backwards. Verified against tfstate, the applied infra is `10.0.0.0/16` with `snet-aca /24`, `snet-pg /28`, `snet-pe /27`. The `10.10.0.0/16` + `/23/24/27` scheme was the abandoned first-pass design and is NOT deployed. `architecture.md` was reconciled to ground truth (diagrams, topology, current/target table; also removed Postgres/ACR which it wrongly showed as deployed). **One thing left to close Module 2:** `docs/services/azure-network.md` is still only a rough outline — must be written into a full service doc using the verified CIDRs above (Brian drafts, Claude edits). **Module 3 (Key Vault) started:** mentor file `docs/mentor/m3-s1-key-vault.md` written (decisions [D1]–[D5] + 5 resources, incl. the project's first private endpoint). **Next action on the other device:** either finish `azure-network.md`, or start Module 3 by writing `infra/environments/prod/keyvault.tf` from scratch per the mentor file (get public IP with `curl -s ifconfig.me` first; `fmt` then send for review before `plan`; expect "4 to add"). Hands-on cadence unchanged: Brian writes new `.tf` from scratch, no [D#] scaffolds.*
